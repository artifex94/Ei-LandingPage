...
export function EventosHeatmap({ eventos, anio }: Props) {
  // Mapa fecha → evento para acceso O(1) desde cada celda de día
  const eventosMap = new Map(eventos.map((e) => [e.fecha, e]));
  ...
  return (
    <div className="portal-panel p-4">
      <div
        ...
        role="list"
        aria-label={`Actividad del sistema de alarma del año ${anio}`}
      >
        {MESES.map((nombreMes, idx) => {
          ...
          return (
            <div key={numMes} role="listitem" aria-label={ariaCard} className="flex flex-col">
              <span className="text-[10px] font-mono font-bold uppercase tracking-widest text-slate-500 px-0.5">
                {nombreMes.slice(0, 3)}
              </span>
              <DayGrid
                mes={numMes}
      ...
      {/* ── Leyenda de colores ── */}
      <div
        className="mt-4 pt-3 border-t border-industrial-700/50 flex flex-wrap gap-x-4 gap-y-1.5"
        aria-label="Referencia de colores del sistema de alarma"
      >
        {LEYENDA.map(({ tipo, label }) => {
          const color = tipo === "vacio" ? "#0f172a" : PALETA[tipo][3];
          return (
...
function DayGrid({ mes, anio, diasEnMes, eventosMap, hoy, nombreMes, hayDatos }: DayGridProps) {
  ...
  return (
    <div className="pt-1 flex-1 flex flex-col gap-1">
      ...
      >
        {Array.from({ length: diasEnMes }, (_, i) => {
          ...
          const tipo: TipoDia = evento?.tipo ?? "vacio";
