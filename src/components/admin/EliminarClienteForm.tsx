...
export function EliminarClienteForm({ id, nombre }: { id: string; nombre: string }) {
  const [confirmando, setConfirmando] = useState(false);
  const [state, action, pending] = useActionState(eliminarCliente, {});
    ...
    <div className="border border-red-900/40 rounded-xl p-5 space-y-3 bg-red-950/20">
      <div>
        ...
        <p className="text-xs text-slate-400 mt-0.5">
          Solo disponible si el cliente no tiene cuentas asociadas. Para clientes con historial, desactivá el perfil en su lugar.
        </p>
      </div>
